...
# run_example = 1 - creating USDBRL total return index rolling forwards and compare with BBG indices
...
if run_example in {1, 0}:
    ...
    df = calculations.join(
        [
            pd.DataFrame(
                df_cuemacro_tot_1M[f'{cross}-forward-tot-1M-cuemacro.close']
            ),
            pd.DataFrame(
                df_cuemacro_tot_3M[f'{cross}-forward-tot-3M-cuemacro.close']
            ),
            df_bbg_tot,
            df_spot,
            df_bbg_tot_forwards,
